...
namespace NFL_QBR_Tracker
{
    public class TestGetStat
    {
        ...
        public static bool RunTest()
        {
            ...
            expected = "INTs per game"; // I think this is what you're expecting. But you should change it if you expected something else
            if (result != expected)
            {
                // If the result is not what we expected, the test should fail
                Console.Error.WriteLine($"Invalid Response: Expected {expected} and the result was {result}");
                return false;
            }
            Console.WriteLine("Running GetStat(List<string> validStats). It should produce a message asking you to enter a QB stat.");
            ...
            expected = "Completion % per game"; // I think this is what you're expecting. But you should change it if you expected something else
            if (result != expected)
            {
                // If the result is not what we expected, the test should fail
                Console.Error.WriteLine($"Invalid Response: Expected {expected} and the result was {result}");
